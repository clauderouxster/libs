...
class Tamguimage;
//This typedef defines a type "imageMethod", which expose the typical parameters of a new Tamgu method implementation
typedef Tamgu* (Tamguimage::*imageMethod)(Tamgu* contextualpattern, short idthread, TamguCall* callfunc);

//---------------------------------------------------------------------------------------------------------------------

class Tamguimage : public TamguObject {
    public:
    //We export the methods that will be exposed for our new object
    //this is a static object, which is common to everyone
    //We associate the method pointers with their names in the linkedmethods map
    static basebin_hash<imageMethod> methods;
    ...
    //---------------------------------------------------------------------------------------------------------------------
    Tamguimage(TamguGlobal* g, Tamgu* parent = NULL) : TamguObject(g, parent) {
        //Do not forget your variable initialisation
		image = NULL;
    }

    Tamguimage() : TamguObject() {
        //Do not forget your variable initialisation
		image = NULL;
    }
    ...
    static void AddMethod(TamguGlobal* g, string name, imageMethod func, unsigned long arity, string infos);
	...
	Tamgu* MethodLoadImageGIF(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {

		if (image != NULL)
			return globalTamgu->Returnerror("WND(805): Image already loaded", idthread);
		Tamgu* kfilename = callfunc->Evaluate(0, contextualpattern, idthread);
		filename = kfilename->String();
		if (filename != "") {
			image = new Fl_GIF_Image(filename.c_str());
			if (image == NULL)
				return globalTamgu->Returnerror("WND(805): Image not loaded", idthread);
			SaveItem(this);
			return aTRUE;
		}
		return aFALSE;
...
#ifndef FLTKNOJPEG
	Tamgu* MethodLoadImageJPEG(Tamgu* contextualpattern, short idthread, TamguCall* callfunc) {
