...
class x_fltk_reading : public x_reading {
public:
    ...
    void setrules() {
        ...
        rules.push_back(".{%a %d}+(=11");               //26    .label(
        rules.push_back(".=#");                         //27    .

        rules.push_back("$%d+=6");                      //28    $%d+
        rules.push_back("$=#");                         //29    $

        rules.push_back("#%d+=6");                      //30    #%d+
        rules.push_back("#=#");                         //31    #

        rules.push_back("?{%a %d}+=6");                 //32    ?label
        rules.push_back("?=#");                         //33    ?

        //Comments are also kept here...
        rules.push_back("//%.~%r+=9");                  //34    comments starting with // with no carriage return (CR) inside
        rules.push_back("//=9");                        //35    comments starting with // with no carriage return (CR) inside
        rules.push_back("/@%.+@/=9");                   //36    long comments starting with /@ and finishing with @/ (not kept)
        rules.push_back("/@@/=9");                      //37    long comments starting with /@ and finishing with @/ (not kept)
        rules.push_back("/=0");                         //38    /

        //Strings
        //Double quote
        rules.push_back("\"\"=1");                      //39    empty string ""
        rules.push_back("\"%?~%r+\"=1");                //40    string "" does not contain CR and can escape characters (%?)

        //Single quote
        rules.push_back("''=2");                        //41    empty string ''
        rules.push_back("'%.~%r+'=2");                  //42    string '' does not contain CR and does not process escape characters
        ...
        //Long quotes
        rules.push_back("@\"\"@=5");                    //43    empty string @""@
        rules.push_back("@\"%?+\"@=5");                 //44    string @" "@ can contain CR and escape characters (we do not keep the @s)
        rules.push_back("@{%a %d}+=13");                //28    @label (head of lexicon annotation rule)

        //Unicode double quote strings
        rules.push_back("u\"\"=1");                     //45    empty string u""
        rules.push_back("u\"%?~%r+\"=1");               //46    string u"" unicode string (we do not keep the u in the parse)

        //Unicode single quote strings
        rules.push_back("u''=2");                       //47    empty string u''
        rules.push_back("u'%.~%r+'=2");                 //48    string u'' unicode string

        //Unicode long quote strings
        rules.push_back("u@\"%?+\"@=5");                //49    empty string u@""@
        rules.push_back("u@\"%?+\"@=5");                //50    string u@".."@ unicode string

        rules.push_back("0x%1+(.%1+)(p([- +])%d+)=3"); //47 hexadecimal
        rules.push_back("%d+(.%d+)(e([- +])%d+)=3");    //52    exponential digits

        // Rules start here
        rules.push_back("{%a %d}+(=12");                //53    label(
        rules.push_back("{%a %d %h}+=4");               //54    label a combination of alpha, digits and hangul characters
        rules.push_back("%n=#");                        //55    non secable space
        rules.push_back("%o=#");                        //56    operators
        rules.push_back("%p=#");                        //55    punctuation
        rules.push_back("%.~{%S %p %o}+=4");            //57    An unknown UTF8 token separated with spaces, punctuation or operators...
    }
...
public:
	...
	void SyntaxColor(int start, int max) {
		...
		x_colorparse.tokenize(localcode);
